...
describe('Register User', () => {
  ...
  before(() => {
    nameSignUp = Cypress.env('nameSignUp')
  })
  ...
  it('Sing up', ()=>{
    ...
    cy.get(singUpUser
      .buttonSingUp).click();
    cy.get(singUpUser
      .emailAlreadyExist).should('not.exist')
  ...
  it('Account information', function (){
    ...
    //Date of birth
    cy.get(enterAccountInformation
      .selectDateDay).select('1');
    cy.get(enterAccountInformation
      .selectDateMonth).select('1');
    cy.get(enterAccountInformation
      .selectDateYear).select('1995');
    //Checkboxes
    cy.get(enterAccountInformation
      .radioButtonNewsLetter).click();
    cy.get(enterAccountInformation
      .radioButtonReceiveSpecialOffers).click();
    //Address information
  ...
  .type(inputTypes
    .testNumber).should('match', /^[0-9]\d*(\.\d+)?$/) //checking the numbers that contain decimals// Este assert no tiene sentido
})
it("Create account", ()=>{
  cy.get(enterAccountInformation
