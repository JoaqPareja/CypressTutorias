   ...
   it('fetches All Products List - GET', () =>{
    ...
    it('Get specific response from body', ()=>{
        cy.get('@starWasrsCharacters')
            ...
            expect(testBody[0]
                    .films[0])
                        .to.eq('https://swapi.dev/api/films/1/');
            expect(testBody[0]
                    .films[1])
                     .to.eq('https://swapi.dev/api/films/2/');
            expect(testBody[0]
                    .films[2])
                        .to.eq('https://swapi.dev/api/films/3/');
            expect(testBody[0]
                    .films[3])
                    .to.eq('https://swapi.dev/api/films/6/')
            expect(testBody[0]
                    .vehicles[0])
                        .to.eq("https://swapi.dev/api/vehicles/14/");
            expect(testBody[0]
                .vehicles[1])
